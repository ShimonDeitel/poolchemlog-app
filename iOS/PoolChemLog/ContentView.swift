...
struct EntryFormView: View {
    ...
    @State private var readingNotes: String = ""
    ...
    init(mode: EntryFormMode) {
        ...
        if case .edit(let entry) = mode {
            ...
            _readingNotes = State(initialValue: entry.readingNotes)
    ...
    var body: some View {
        NavigationStack {
            Form {
                ...
                TextField("Chlorine / pH Notes", text: $readingNotes)
            ...
            .toolbar {
                ...
                ToolbarItem(placement: .confirmationAction) {
                    ...
                    .disabled(readingNotes.trimmingCharacters(in: .whitespaces).isEmpty)
                }
            }
        }
    }
    ...
    private func save() {
        switch mode {
        case .add:
            let entry = ReadingEntry(date: date, readingNotes: readingNotes, notes: notes)
            store.add(entry)
        case .edit(let existing):
            var updated = existing
            updated.date = date
            updated.readingNotes = readingNotes
            updated.notes = notes
